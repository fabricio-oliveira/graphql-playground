...
class BuilderServer {
  ...
  setRoute(method, path, cb) {
    if (method == "GET") this.router.get(path, cb);
    else if (method == "POST") this.router.post(path, cb);
    else if (method == "PUT") this.router.put(path, cb);
    else if (method == "DELETE") this.router.delete(path, cb);
  ...
  async build() {
    ...
    if (this.enableGraphqlSubscriber) {
      if (!this.graphqlRootPath)
        throw new Error("grapqlRootPath nees to be defined");
      ...
      this.serverStart = {
        ...
        async serverWillStart() {
          return {
            async drainServer() {
              console.log('😫 Graphql ws shutdown');
    ...
    if (this.enableGraphqlServer) {
      ...
      this.app.use(
        this.graphqlRootPath,
        expressMiddleware(gqlServer, { context: async () => this.context })
      );
    }
    ...
    httpServer.listen(this.port, () => {
      ...
      if (this.enableGraphqlServer)
        console.log(
          `🚀 Graphql server ready at http://localhost:${this.port}${this.graphqlRootPath}`
        );
    });
  }
}
